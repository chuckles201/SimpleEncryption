...
// xor
void x_or( uint8_t inArray[4],uint8_t outArray[4]) {
    for (int i = 0; i < 4; i++) {
...
void sub_bytes(uint8_t inArray[4],uint8_t outArray[4]) {
...
// all zeros double val...

void genrc(uint8_t result_out[NUM_KEYS-2]) {
    // len of 4 (words), for each-key!
    uint8_t result[NUM_KEYS-2] = {0}; // init at zeros
    ...
    for (int i=0; i < NUM_KEYS-2; i++) {
        ...
        if (overflow) {
            overflow = 0;
            // xor with irreducible, mods #!
            n ^= irreducible;
        }
    }
    memcpy(result_out,result,(NUM_KEYS-2));
}
...
void keyExpansion(uint8_t inKey[32],uint8_t full_key[60][4]) {
    ...
    for (int word = 8; word < 15*4; word++){
        if (word % 8 == 0){ // do full operation
            ...
            sub_bytes(full_key[word],full_key[word]);
            x_or(rc[(int)(word-8) / 8],full_key[word]); 

            x_or(full_key[word-8],full_key[word]);
            ...
            printf("\n%d perc4\n",word);
            x_or(full_key[word-1],full_key[word]);
            sub_bytes(full_key[word-1],full_key[word]);
            x_or(full_key[word-8],full_key[word]);
...
int main () {
    // copying block...
    uint8_t key_begin[SIZE] = {
        // 8 cols/'words'
        0x60,0x3d,0xeb,0x10,0x15,0xca,0x71,0xbe,0x2b,0x73,0xae,0xf0,0x85,0x7d,0x77,0x81,
        0x1f,0x35,0x2c,0x07,0x3b,0x61,0x08,0xd7,0x2d,0x98,0x10,0xa3,0x09,0x14,0xdf,0xf4
        
    }; uint8_t stateFinal[60][4] = {{0}};
    // testing
    uint8_t testArr[4] = {0x09,0x14,0xdf,0xf4};
    uint8_t testArrPrev[4] = {0x60,0x3d,0xeb,0x10};
    printf("\n<TESTS>\n\n");
    rot_col(testArr,testArr);
    printf("Rotate-bytes: %02x,%02x,%02x,%02x\n",testArr[0],testArr[1],testArr[2],testArr[3]);
    sub_bytes(testArr,testArr);
    printf("Sub-Bytes:    %02x,%02x,%02x,%02x\n",testArr[0],testArr[1],testArr[2],testArr[3]);
    x_or(rc[0],testArr);
    x_or(testArrPrev,testArr);
    printf("Rcon:         %02x,%02x,%02x,%02x\n",testArr[0],testArr[1],testArr[2],testArr[3]);
    printf("\n</TESTS>\n\n");

    
    keyExpansion(key_begin,stateFinal);
    for(int k = 0; k <15;k++){
        printf("\nKey *%d*\n",k);
        for (int r = 0; r < 4; r++) {
            for (int c=0; c< 4; c++){
                printf("%02x ",stateFinal[k*4+c][r]);
            }printf("\n");
        }
    }
